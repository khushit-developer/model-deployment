...
protoPath = os.path.sep.join([settings.BASE_DIR, "face_detector/MobileNetSSD_deploy.prototxt"])
weiPath = os.path.sep.join([settings.BASE_DIR,"face_detector/MobileNetSSD_deploy.caffemodel"])
net = cv2.dnn.readNet(protoPath, weiPath)
...
run=dill.load(open('modelpik.sav','rb)
run(CLASSES,net,vs)